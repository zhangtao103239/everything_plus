  ...
  async def embed_text(self, texts: list, batch_size=15) -> list:
    ...
    for i in range(0, len(texts), batch_size):
      ...
      async with aiohttp.ClientSession() as session:
        ...
        async with session.post(self.config.embedding_url, headers=headers, json=data) as resp:
          if resp.status == 413:
          ...
          self.logger.debug(f"Embedded batch {i//batch_size+1}/{len(texts)//batch_size+1}")
...
class SearchApp:
  ...
  def create_widgets(self):
    ...
    # Embedding服务
    ttk.Label(config_frame, text="Embedding URL:").grid(row=0, column=0, padx=5, pady=5)
    ...
    # Reranker服务
    ttk.Label(config_frame, text="Reranker URL:").grid(row=1, column=0, padx=5, pady=5)
    ...
    # 结果区域
    self.results = ttk.Treeview(main_frame, columns=('文件', '内容'), show='headings')
    ...
    self.log_display = scrolledtext.ScrolledText(log_frame, state='disabled', height=10)
  ...
  def start_observer(self):
    ...
    self.observer.schedule(FileChangeHandler(self.processor), path="/", recursive=True)
    self.observer.start()
  ...
  def perform_search(self):
    ...
    self.logger.info(f"Searching for '{query}' returned {len(results['ids'][0])} results")
